...
do
    ...
    # ⚡ 步骤 1: 训练 (Conv-TimeGAN: window=60, hidden=24, iter=5000, lr=0.0001)
    echo "🏗️ Phase 1: Training..."
    python train.py --data_name "$app" --seq_len 60 --hidden_dim 24 --batch_size 128 --iteration 5000 --lr 0.0001
    
    if [ $? -eq 0 ]; then
        echo -e "\033[0;32m✅ Training Finished: $app\033[0m"
        
        # ⚡ 步骤 2: 采样 (采用 60 窗口大小，然后在 sample_only.py 中被拼接成 512)
