...
for app_name, app_conf in CONFIG['appliances'].items():
    params_appliance[app_name] = {
        'mean': app_conf['mean'],
        'std': app_conf['std'],
        'houses': app_conf['train']['houses'],
        'channels': app_conf['train']['channels']
    }
...
def main():
    ...
    validation_percent = CONFIG['processing']['training']['validation_percent']
    testing_percent = CONFIG['processing']['training']['testing_percent']
    nrows = None
    ...
    for h in params_appliance[appliance_name]['houses']:
        ...
        # ── DATE FILTERING ──
        start_date = CONFIG['processing']['training'].get('start_date')
        end_date = CONFIG['processing']['training'].get('end_date')
        if start_date:
