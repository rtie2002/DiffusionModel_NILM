...
from tensorflow.keras.layers import Dense, Conv2D, Conv1D,Flatten, Reshape,UpSampling2D,Add,Concatenate,BatchNormalization
...
def get_model(appliance, input_tensor, window_length, transfer_dense=False, transfer_cnn=False,
    ...
    cnn2 = Conv1D(filters=16, kernel_size=3, strides=1, padding='same', activation='relu')(cnn1)
    cnn2 = BatchNormalization()(cnn2)

    flat = Flatten(name='flatten')(cnn2)

    d = Dense(512, activation='relu', name='dense')(flat)
    d = BatchNormalization()(d)

    if n_dense == 1:
        label = d
    elif n_dense == 2:
        d1 = Dense(512, activation='relu', name='dense1')(d)
        d1 = BatchNormalization()(d1)
        label = d1
    elif n_dense == 3:
        d1 = Dense(512, activation='relu', name='dense1')(d)
        d1 = BatchNormalization()(d1)
        d2 = Dense(512, activation='relu', name='dense2')(d1)
        d2 = BatchNormalization()(d2)
        label = d2

    d_out = Dense(window_length, activation='linear', name='output')(label)
...
def get_model1(appliance, input_tensor, window_length, transfer_dense=False, transfer_cnn=False,
    ...
    cnn1 = Conv2D(filters=8, kernel_size=(5, 1), strides=(1, 1), padding='same', activation='relu')(reshape)
    cnn2 = Conv2D(filters=8, kernel_size=(5, 1), strides=(1, 1), padding='same', activation='relu')(cnn1)
    cnn2 = BatchNormalization()(cnn2)

    cnn3 = Conv2D(filters=16, kernel_size=(3, 1), strides=(1, 1), padding='same', activation='relu')(cnn2)
    cnn4 = Conv2D(filters=16, kernel_size=(3, 1), strides=(1, 1), padding='same', activation='relu')(cnn3)
