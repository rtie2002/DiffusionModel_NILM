...
def algorithm1_data_cleaning(power_sequence, x_threshold, l_window=100, x_noise=0,
    ...
    Input:
        power_sequence: power sequence x = [x1,...,xt,...] (in Watts)
...
def main():
    ...
    parser.add_argument('--remove_spikes', action='store_true', default=True,
                        help='Remove isolated spikes that appear when surrounding is near zero (default: True)')
    ...
    parser.add_argument('--spike_threshold', type=float, default=3.0,
                        help='Spike threshold multiplier (default: 3.0, higher = less sensitive)')
    parser.add_argument('--background_threshold', type=float, default=50,
                        help='Background threshold in Watts to consider surrounding as "low" (default: 50W)')
    ...
    if args.remove_spikes:
        print(f"    Spike window: {args.spike_window}")
        print(f"    Spike threshold: {args.spike_threshold}x median")
        print(f"    Background threshold: {args.background_threshold}W")
